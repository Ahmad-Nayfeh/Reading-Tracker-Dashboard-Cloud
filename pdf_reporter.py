...
class PDFReporter(FPDF):
    ...
    def add_kpi_grid(self, kpis: dict):
        ...
        num_cols = 3
        col_width = (drawable_width - (num_cols - 1) * 5) / num_cols # 5mm gap
        card_height = 30
        icon_size = 18
        
        kpi_list = list(kpis.items())
        for i in range(0, len(kpi_list), num_cols):
            # Auto page break if the row doesn't fit
            if self.get_y() + card_height > (self.h - self.b_margin):
                self.add_page_with_background(use_background=False)
            ...
            for label, (value, icon) in row_items:
                ...
                self.rect(x, y, col_width, card_height, 'F')
                
                # Draw the accent line on the right
                self.set_draw_color(*ACCENT_COLOR)
                self.line(x + col_width, y, x + col_width, y + card_height)

                # Icon
                self.set_font("Amiri", "", icon_size)
                self.set_text_color(*ACCENT_COLOR)
                # Center the icon vertically in its space
                self.set_xy(x + col_width - icon_size - 5, y + (card_height - icon_size) / 2)
                ...
                self.set_x(x + col_width + 5) # Move to next card position with gap
            self.ln(card_height + 5) # New line with gap
    ...
    def add_hall_of_fame_grid(self, heroes: dict):
        ...
        drawable_width = self._get_drawable_width()
        num_cols = 4
        col_width = (drawable_width - (num_cols - 1) * 4) / num_cols # 4mm gap
        card_height = 35
        ...
        for i in range(0, len(heroes_list), num_cols):
            ...
            for title, (name, value) in row_items:
                x = self.get_x()
                y = self.get_y()
                ...
                self.rect(x, y, col_width, card_height, 'DF')
                ...
                self.set_x(x + col_width + 4) # Move to next card position
            self.ln(card_height + 5) # New line
