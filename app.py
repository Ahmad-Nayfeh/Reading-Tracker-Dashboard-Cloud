        ...
        if st.button("📝 إنشاء الشيت والفورم الآن", type="primary", use_container_width=True):
            try:
                ...
                if 'invalid_grant' in str(e) or 'revoked' in str(e):
                    # 1. أولاً، ندمر الجلسة الشبح بالخروج
                    auth_manager.logout()
                    
                    # 2. ثانياً، نعرض للمستخدم التعليمات الواضحة
                    st.error("⚠️ خطأ حرج في الصلاحيات!")
                    st.warning(
                        "لقد اكتشفنا أن صلاحيات وصول التطبيق لحسابك قد تم إلغاؤها. "
                        "هذا يحدث عادة بعد حذف الحساب وإعادة التسجيل مباشرة."
                    )
                    st.markdown("---")
                    st.markdown("#### **الحل (خطوتان فقط):**")
                    st.markdown("1. **اذهب إلى صفحة أذونات جوجل من الرابط أدناه وقم بإزالة وصول 'ماراثون القراءة'.**")
                    st.markdown("[🔗 **اضغط هنا للذهاب إلى صفحة أذونات حساب جوجل**](https://myaccount.google.com/permissions)", unsafe_allow_html=True)
                    st.markdown("2. **بعد إزالة الوصول، عد إلى هنا وقم بتحديث هذه الصفحة (اضغط F5). سيتم تسجيل خروجك تلقائياً للبدء من جديد.**")
                    
                    # 3. أخيراً، نوقف تنفيذ الكود لمنع أي أخطاء أخرى
                    st.stop()
                else:
