...
if not creds or not user_id:
    # This case handles when the user is properly logged out and needs to re-authenticate.
    # The authenticate() function will show the login button.
    st.stop()
        ...
        if st.button("📝 إنشاء الشيت والفورم الآن", type="primary", use_container_width=True):
            try:
                ...
                if 'invalid_grant' in str(e) or 'revoked' in str(e):
                    st.error("⚠️ خطأ في الصلاحيات: فشل الاتصال بخدمات جوجل.")
                    st.warning(
                        "هذا يحدث عادة بعد حذف الحساب وإعادة التسجيل مباشرة. "
                        "لحل المشكلة، يجب إعادة ضبط جلسة التطبيق بالكامل."
                    )
                    st.info("الرجاء الضغط على الزر أدناه للبدء من جديد.")

                    if st.button("🚪 تسجيل الخروج والبدء من جديد", use_container_width=True, type="primary"):
                        auth_manager.logout()
                        st.rerun()
                else:
